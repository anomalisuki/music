...
// ---- AUDIO ENGINE (elemen <audio> native, sumber stream dari https://nanzz-music.netlify.app/api/ytplay) ----
    ...
    fetch(API.ytplay,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:ytUrl})})
        ...
        .then(function(d){
            if(d&&d.status&&d.result&&d.result.download&&d.result.download.audio){
                ...
                var a=document.createElement('a');
                a.href='https://nanzz-music.netlify.app/api/proxy-audio?url='+encodeURIComponent(audioUrl);
