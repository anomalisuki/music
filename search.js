var Search = {
    ...
    REC_ROWS: [{
        ...
        label: 'Rilis Anyar',
        q: 'baru rilis',
        icon: '<i data-lucide="sparkles" class="w-4 h-4 text-amber-400"></i>'
    }, {
        key: 'rec1',
        label: 'Barat Top',
        q: 'barat Top',
        icon: '<i data-lucide="globe" class="w-4 h-4 text-sky-400"></i>'
    }, {
        key: 'rec2',
        label: 'Rapp Top',
        q: 'Rapp Top',
        icon: '<i data-lucide="flame" class="w-4 h-4 text-rose-400"></i>'
    }],
    renderRecs() {
        var rc = gid('search-recs');
        if (!rc)
            return;
        if (S.rec0 && S.rec1 && S.rec2) {
            Search.showRecs();
            return;
        }
        rc.innerHTML = Search.REC_ROWS.map(function(row) {
            return '<div><div class="h-5 w-32 bg-white/10 rounded mb-3 animate-pulse"></div><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-1">' + Array(4).fill(0).map(function() {
                return '<div class="flex-shrink-0 w-32 animate-pulse"><div class="w-32 h-32 rounded-xl bg-white/5 mb-2"></div><div class="h-3 bg-white/10 rounded w-3/4"></div></div>';
            }).join('') + '</div></div>';
        }).join('');
        Promise.all(Search.REC_ROWS.map(function(row) {
            return fetch(API.search + '?query=' + encodeURIComponent(row.q) + '&type=songs').then(function(r) {
                return r.json();
            }).then(function(d) {
                S[row.key] = d.status && d.result.songs ? d.result.songs.map(function(s) {
                    return {
                        id: s.videoId,
                        videoId: s.videoId,
                        title: cn(s.title),
    ...
    renderActive() {
        ...
        if (c && S.sq && S.filter === 'songs' && S.sr) {
            ...
            items.forEach(function(el, i) {
                ...
                if (titleEl)
                    titleEl.className = 'search-song-title font-medium truncate ' + titleColor;
            ...
            Search.REC_ROWS.forEach(function(row) {
                ...
                list.forEach(function(t, i) {
                    ...
                    if (card) {
                        ...
                        if (cover)
                            cover.className = 'search-rec-cover w-32 h-32 mb-2 relative rounded-xl overflow-hidden glass-edge  transition-all ' + ringClass;
                        var btn = card.querySelector('.search-rec-btn');
                        if (btn)
                            btn.innerHTML = recBtn;
                        var title = card.querySelector('.search-rec-title');
                        if (title)
                            title.className = 'search-rec-title font-semibold text-xs truncate ' + (isCur ? 'text-white font-black' : '');
                    }
                });
            });
        }
    ...
    updateFilterUI() {
        ...
        if (a) {
            a.className = 'filter-tab active flex-1 py-2 px-4 rounded-full text-xs font-bold btn-chrome text-white  border border-white/30 scale-105';
        }
    },
    show(loading) {
        var c = gid('search-results')
        ...
        if (loading) {
            c.innerHTML = '<div class="text-center mt-10"><div class="w-8 h-8 border-3 border-[#cfd3d8] border-t-transparent rounded-full animate-spin mx-auto"></div></div>';
            return;
        }
        if (S.sr.length === 0) {
            c.innerHTML = '<p class="text-center text-white/70 mt-10">Tidak ada hasil</p>';
            return;
        }
        ...
        if (S.filter === 'songs') {
            c.innerHTML = S.sr.map(function(t, i) {
                ...
                return '<div onclick="PK(\'search\',' + i + ')" class="search-song-item flex items-center gap-3.5 p-2.5 mb-2 rounded-2xl cursor-pointer active:scale-[0.98] transition-all shadow-lg shadow-black/25 animate-card-up ' + itemBg + '" style="animation-delay:' + Math.min(i * 35, 500) + 'ms">' + '<img src="' + toWebp(t.cover) + '" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" onerror="handleImgError(this)" />' + '<div class="truncate flex-1 min-w-0"><div class="flex items-center"><h3 class="search-song-title font-semibold text-sm truncate ' + titleColor + '">' + es(t.title) + '</h3><span class="search-song-badge">' + badgeHtml + '</span></div><p class="text-white/60 text-xs truncate mt-0.5">' + es(t.artist) + '</p></div>' + '<div class="search-song-btn">' + btnHtml + '</div>' + '</div>';
            }).join('');
            lucide.createIcons();
